...
public class SEIRModel{
    ...
    public static Data RunModel(CountyDataArray SeriesData,Data CountyData, int Imported, int Exported, int ResponseTier, int County, int Local_workers, int Planned_vaccination, double[][] Age_specific_vaccine_distribution){
        ...
        for (int Patient_Age_Band = 0; Patient_Age_Band < 16; Patient_Age_Band++) {
            double Age_Immuned = (CountyData.getDataPackByAge()[0][2][Patient_Age_Band] + CountyData.getDataPackByAge()[1][2][Patient_Age_Band]) + (Parameters.Efficacy_Against_Infection * (CountyData.getDataPackByAge()[0][12][Patient_Age_Band] + CountyData.getDataPackByAge()[1][12][Patient_Age_Band]));
            double ShieldingImm = Age_Immuned/CountyData.getDataPackByAge()[0][1][Patient_Age_Band];
